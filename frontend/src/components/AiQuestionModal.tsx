...
export default function AiQuestionModal({
  ...
  // Document sections state (Loaded automatically when document is selected)
  ...
  // Load document sections whenever selectedDocId changes
  useEffect(() => {
    if (selectedDocId) {
      ...
      api
        ...
        .then((res) => {
          ...
          setSelectedChapters([]);
          setSelectedTopics([]);
          // Auto fill subject & grade from document if matching
  ...
  // Helper logic for topics filtering
  const availableTopics = docSections
    ? Array.from(
        new Set(
          docSections.chapters
            .filter(
              (ch) =>
                selectedChapters.length === 0 || selectedChapters.includes(ch.title)
            )
            .flatMap((ch) => ch.lessons)
            .concat(selectedChapters.length === 0 ? docSections.all_topics : [])
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          {!result ? (
            <form onSubmit={handleGenerate} className="space-y-6">
              ...
              {/* Document Selection (Grounding Source) */}
              <div>
              ...
              {/* Section & Topic Selection Area */}
              {selectedDocId ? (
                <div className="rounded-2xl border border-purple-200 bg-purple-50/30 p-4 space-y-4">
                  {loadingSections ? (
                    <div className="flex items-center justify-center gap-2 py-6 text-xs text-purple-700 font-medium">
                      <RefreshCw className="w-4 h-4 animate-spin text-purple-600" />
                      Đang đọc cấu trúc các Đầu mục & Chương từ tài liệu...
                    </div>
                  ) : docSections && (docSections.all_chapters.length > 0 || docSections.all_topics.length > 0) ? (
                    <div className="space-y-4">
                      {/* Summary Banner */}
                      <div className="flex items-center justify-between text-xs border-b border-purple-100 pb-2">
                        <span className="font-bold text-purple-950 flex items-center gap-1.5">
                          <ListChecks className="w-4 h-4 text-purple-600" /> Đầu Mục Tri Thức Đã Trích Xuất ({docSections.title})
                        </span>
                        {(selectedChapters.length > 0 || selectedTopics.length > 0) && (
                          <span className="bg-purple-600 text-white px-2.5 py-0.5 rounded-full font-bold text-[11px]">
                            Đã chọn: {selectedChapters.length} Chương, {selectedTopics.length} Bài học
                          </span>
                        )}
                      </div>

                      {/* Chapters Selection */}
                      {docSections.all_chapters.length > 0 && (
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                              <FolderOpen className="w-3.5 h-3.5 text-purple-600" /> Chọn Các Chương / Đầu Mục Lớn ({selectedChapters.length}/{docSections.all_chapters.length})
                            </span>
                            <div className="flex items-center gap-3 text-[11px]">
                              <button
                                type="button"
                                onClick={() => setSelectedChapters([...docSections.all_chapters])}
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-1 border border-slate-200/80 rounded-xl bg-white">
                            {docSections.all_chapters.map((chTitle) => {
                              const isSelected = selectedChapters.includes(chTitle);
                              return (
                                <label
                                  key={chTitle}
                                  className={`flex items-start gap-2.5 p-2.5 rounded-lg border text-xs cursor-pointer transition ${
                                    isSelected
                                      ? 'border-purple-500 bg-purple-50 text-purple-950 font-bold shadow-sm ring-1 ring-purple-500/20'
                                ...
                                >
                                  <input
                                    ...
                                    onChange={() => {
                                      if (isSelected) {
                                        setSelectedChapters((prev) => prev.filter((c) => c !== chTitle));
                                      } else {
                                        setSelectedChapters((prev) => [...prev, chTitle]);
                                      }
                                    }}
                                    className="w-4 h-4 rounded text-purple-600 focus:ring-purple-500 border-slate-300 mt-0.5 shrink-0"
                                  />
                                  <span className="leading-snug">{chTitle}</span>
                                </label>
                              );
                            })}
                          </div>
                      ...
                      {/* Lessons / Topics Selection */}
                      {availableTopics.length > 0 && (
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                              <FileText className="w-3.5 h-3.5 text-indigo-600" /> Chọn Các Bài Học / Chủ Đề Con ({selectedTopics.length}/{availableTopics.length})
                            </span>
                            <div className="flex items-center gap-3 text-[11px]">
                              <button
                                type="button"
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-1 border border-slate-200/80 rounded-xl bg-white">
                            {availableTopics.map((topTitle) => {
                              const isSelected = selectedTopics.includes(topTitle);
                              return (
                                <label
                                  key={topTitle}
                                  className={`flex items-start gap-2.5 p-2.5 rounded-lg border text-xs cursor-pointer transition ${
                                    isSelected
                                      ? 'border-indigo-500 bg-indigo-50 text-indigo-950 font-bold shadow-sm ring-1 ring-indigo-500/20'
                                      : 'border-slate-100 bg-slate-50/50 hover:bg-slate-100 text-slate-700'
                                  }`}
                                >
                                  <input
                                    ...
                                    onChange={() => {
                                      if (isSelected) {
                                        setSelectedTopics((prev) => prev.filter((t) => t !== topTitle));
                                      } else {
                                        setSelectedTopics((prev) => [...prev, topTitle]);
                                      }
                                    }}
                                    className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 mt-0.5 shrink-0"
                                  />
                                  <span className="leading-snug">{topTitle}</span>
                                </label>
                              );
                            })}
                          </div>
                    ...
                    <div className="text-xs text-slate-500 text-center py-4">
                      Không tìm thấy danh mục chương cụ thể trong tài liệu. AI sẽ đọc trực tiếp nội dung các trang để tạo câu hỏi.
                    </div>
                  )}
                </div>
              ) : (
                /* Fallback Manual Chapter & Lesson Inputs when no document is attached */
